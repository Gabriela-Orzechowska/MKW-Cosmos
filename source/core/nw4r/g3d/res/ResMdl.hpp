...
public:
    ResMat GetResMat(int idx) const; //offset to Dic = ResDic->ResMat, same for all the get
    ResMat GetResMat(const char *name) const; //8004e8f0
    u32 GetResMatNumEntries() const; //8004ea50
    ResTexPlttInfoOffset GetResTexPlttInfoOffsetFromTexName(u32 idx); //8004eb90 "Mdl0::RawTextureLinkByID"
