...
static_assert(sizeof(LecodeSELECTPacket) == 0x38, "LecodeSELECTPacket");
...
class LecodeRKNetSELECTHandler{
public:
    static LecodeRKNetSELECTHandler *sInstance; //0x809c2100
    static LecodeRKNetSELECTHandler *GetStaticInstance(); //8065fe8c
    static void DestroyStaticInstance(); //8065ff60
    LecodeRKNetSELECTHandler(); //8066076c inlined
    ~LecodeRKNetSELECTHandler(); //806607f4
    void InitPackets(); //8065ffa4 sets datas to invalid/not selected etc... composed of a bunch of inlined functions
    u8 GetWinningVoter() const; //80660434
    CourseId GetWinningCourse() const; //80660450
    u32 GetLocalSelectId() const; //80660470 from sent packet
    ...
    OnlineMode mode; //from page 0x90 OnInit MenuId Switch
    u32 unknown_0x4;
    LecodeSELECTPacket toSendPacket; //0x8
    LecodeSELECTPacket receivedPackets[12]; //0x40
    u8 lastSentToAid; //0x2e0
    u8 unknown_0x2e4[7];
    u64 lastSentTime; //0x2e8
    u64 lastReceivedTimes[12]; //0x2f0
...
static_assert(sizeof(LecodeRKNetSELECTHandler) == 0x3F8, "LecodeRKNetSELECTHandler");
