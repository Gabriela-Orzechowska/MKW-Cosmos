...
public:
    ...
    inline Aurora::ScenePlus* GetCurrentScene() { return curScene; } 
    inline GlobalContext* GetCurrentContext() { return globalContext; }
    
    template <class T>
    inline T* GetPage(PageId pageId) { return curScene->Get<T>(pageId); }

    template <class T>
    inline T* GetAuroraPage(Aurora::AuroraPageId pageId) { return curScene->GetAuroraPage<T>(pageId);}
    
    template <class T>
    inline T* GetAuroraPage(PageId pageId) { return curScene->GetAuroraPage<T>((Aurora::AuroraPageId)pageId);}
    ...
    void LoadNextMenu();
    Aurora::ScenePlus* curScene; /* http://wiki.tockdom.com/wiki/List_of_Identifiers#Menu_Identifiers */ //0x0
