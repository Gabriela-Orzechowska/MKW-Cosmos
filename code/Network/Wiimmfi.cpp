...
extern "C" {
    ...
    int NHTTPGetBodyAll(void * handle, char ** responseBody);
}

u32 expectedHash[] ={ 0x0FFF1F07, 0x00E638C9, 0x49FBEFFA, 0x79022D3A, 0x84AB134F };

/*
int WiimmfiInject1(void * handle, int * responseBody)
{
    int ret = NHTTPGetBodyAll(handle, (char **) responseBody);
    if(ret < 0) {
        return -1;
    }
    ...
    if(DWC_AuthServer == 2){
        // PAYLOAD
        u8 byte = *(u8 *)((((u32)responseBody) + 0x3) & ~0x3);
        register u32 payloadAddr = ((u32) responseBody) + byte;
        asm{
            ASM(
                dcbf 0, payloadAddr;
                mtlr payloadAddr;
    ...
    SSL_Initialised = *responseBody;
    DWC_AuthServer = 2;

    return -1;
}

kmCall(0x800ee39c, WiimmfiInject1);
*/
