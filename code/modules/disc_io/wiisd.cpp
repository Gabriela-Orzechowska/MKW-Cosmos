...
static int __sdio_sendcommand(u32 cmd,u32 cmd_type,u32 rsp_type,u32 arg,u32 blk_cnt,u32 blk_size,void *buffer,void *reply,u32 rlen)
{
	int ret;
	STACK_ALIGN(IOS::IOCtlvRequest,iovec,3,32);
	...
	if(request->isdma || __sd0_sdhc == 1) {
		iovec[0].address = request;
		iovec[0].size = sizeof(struct _sdiorequest);
		iovec[1].address = buffer;
		iovec[1].size = (blk_size*blk_cnt);
		iovec[2].address = response;
		iovec[2].size = sizeof(struct _sdioresponse);
		ret = IOS::IOCtlv(__sd0_fd,(IOS::IOCtlType) IOCTL_SDIO_SENDCMD,2,1,iovec);
	} else
		ret = IOS::IOCtl(__sd0_fd,(IOS::IOCtlType) IOCTL_SDIO_SENDCMD,request,sizeof(struct _sdiorequest),response,sizeof(struct _sdioresponse));
...
static int __sdio_setclock(u32 set)
{
	int ret;
 	STACK_ALIGN(u32,clock,1,32);

	*clock = set;
	ret = IOS::IOCtl(__sd0_fd,(IOS::IOCtlType) IOCTL_SDIO_SETCLK,clock,sizeof(u32),NULL,0);
...
static int __sdio_getstatus()
{
	...
	ret = IOS::IOCtl(__sd0_fd,(IOS::IOCtlType) IOCTL_SDIO_GETSTATUS,NULL,0,status,sizeof(u32));
	if(ret<0) return ret;
...
static int __sdio_resetcard()
{
	...
	__sd0_rca = 0;
	ret = IOS::IOCtl(__sd0_fd,(IOS::IOCtlType) IOCTL_SDIO_RESETCARD,NULL,0,status,sizeof(u32));
	if(ret<0) return ret;
...
static int __sdio_gethcr(u8 reg, u8 size, u32 *val)
{
	int ret;
	...
	*val = 0;
	...
	hcr_query[4] = 0;
	hcr_query[5] = 0;
	ret = IOS::IOCtl(__sd0_fd,(IOS::IOCtlType) IOCTL_SDIO_READHCREG,(void*)hcr_query,24,hcr_value,sizeof(u32));
	*val = *hcr_value;

	return ret;
}
 
static int __sdio_sethcr(u8 reg, u8 size, u32 data)
{
	int ret;
	STACK_ALIGN(u32,hcr_query,6,32);
	...
	hcr_query[4] = data;
	hcr_query[5] = 0;
	ret = IOS::IOCtl(__sd0_fd,(IOS::IOCtlType) IOCTL_SDIO_WRITEHCREG,(void*)hcr_query,24,NULL,0);


	return ret;
}

static int __sdio_waithcr(u8 reg, u8 size, u8 unset, u32 mask)
{
	u32 val;
	int ret;
	int tries = 10;
...
static int __sdio_setbuswidth(u32 bus_width)
{
	int ret;
	u32 hc_reg = 0;
...
static int __sd0_setblocklength(u32 blk_len)
{
	int ret;
...
static int __sd0_setbuswidth(u32 bus_width)
{
	u16 val;
	int ret;
...
extern void * iosAlloc(int hid, s32 size, u32 align); //80194ae8
extern int iosCreateHeap(void * buffer, s32 size); //801949b8
